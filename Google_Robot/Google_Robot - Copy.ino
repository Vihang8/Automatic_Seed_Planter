...
#define username_ada    "VihangB"            // Replace it with your username
#define ssid       "A103-DND"             // Your SSID
#define password       "Mumbai2FortCollins"        // Your password
//--------------------------------------------------------------//
...
int calculateDistance_seed()
{
  digitalWrite(trigPin_seed,LOW);
  delayMicroseconds(2);
  digitalWrite(trigPin_seed,HIGH);
  delayMicroseconds(10);
  digitalWrite(trigPin_seed,LOW);
  duration_seed=pulseIn(echoPin_seed,HIGH);
  distance_seed=duration_seed*0.34/2;
  return distance_seed;
}  

int calculateDistance_water_level()
{
  digitalWrite(trigPin_water_level,LOW);
  delayMicroseconds(2);
  digitalWrite(trigPin_water_level,HIGH);
  delayMicroseconds(10);
  digitalWrite(trigPin_water_level,LOW);
  duration_water_level=pulseIn(echoPin_water_level,HIGH);
  distance_water_level=duration_water_level*0.34/2;
  return distance_water_level;
}  
